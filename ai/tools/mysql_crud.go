...
func NewMysqlCrudTool() tool.InvokableTool {
	t, err := utils.InferOptionableTool(
		...
		func(ctx context.Context, input *MysqlCrudInput, opts ...tool.Option) (output string, err error) {
			...
			fmt.Println()
			nInput := scanner.Text()
			if nInput != "y" && nInput != "Y" { //大小写都行
				return "用户取消执行sql", nil
			}

			// 2. 执行 SQL
			if input.OperateType == "query" {
				// 3. 获取查询结果（每一行是一个 map，列名做 key，适合动态列的 SELECT *）
			...
			if input.OperateType == "insert" || input.OperateType == "update" || input.OperateType == "delete" {
			...
			return "invalid operate type", nil
		})
	if err != nil {
		log.Fatal(err)
	}
