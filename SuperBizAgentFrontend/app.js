...
class SuperBizAgentApp {
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '问问 WatchTower 值班助手';
        }
    }
        ...
        try {
            ...
            try {
                while (true) {
                    ...
                    if (done) {
                        this.finalizeStreamMessage(assistantMessageElement, fullResponse);
                        break;
                    }
                    ...
                    // SSE spec: consecutive "data:" lines within one event
                    // are joined with \n. An empty line marks event boundary.
                    ...
                    for (const line of lines) {
                        ...
                        if (line.startsWith('id: ')) {
                            ...
                            const data = line.startsWith('data: ') ? line.substring(6) : '';
                            if (data === '[DONE]') {
                                this.finalizeStreamMessage(assistantMessageElement, fullResponse);
                                return;
                            }
    ...
    // 流式消息完成：转 Markdown 渲染 + 保存历史
    finalizeStreamMessage(assistantMessageElement, fullResponse) {
        if (assistantMessageElement) {
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/ai-ops`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            if (data.result !== undefined) {
                ...
                try {
                    ...
                    responseText = resultObj.response || data.result;
                } catch (e) {
                    responseText = data.result;
                }
                
                this.updateAIOpsMessage(loadingMessageElement, responseText, data.detail || []);
            } else {
                throw new Error(data.error || '未知错误');
            }
        } catch (error) {
